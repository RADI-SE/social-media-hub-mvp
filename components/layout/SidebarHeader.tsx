...
const growthIcons: Record<string, LucideIcon> = {
  ...
  growthPipeline: TrendingUp,
  growthROI: PieChart,
  growthLTV: DollarSign,
  growthAlerts: Bell,
  growthRecommendations: Lightbulb,
...
export default function SidebarHeader() {
  ...
  const growthItems = role
    ? dashboardNavigation[role].map(({ label, href }) => ({
      name: t(label),
      href,
  ...
  return (
    <nav className="mt-8" aria-label={t("workspace")}>
      ...
      {/* Show "New" dropdown only for social media users when role is known */}
      ...
      {/* If role is null, NavItems will receive an empty array and render nothing */}
