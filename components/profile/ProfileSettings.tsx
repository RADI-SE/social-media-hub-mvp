...
export function ProfileSettings() {
  ...
  async function saveName() {
    ...
    try {
      ...
      setNameError(messageFrom(error, "Could not update your name."));
    } finally {
      setSavingField(null);
    }
  }
  ...
  function requestEmailChange() {
    ...
    if (!EMAIL_PATTERN.test(normalized)) {
      setEmailError("Enter a valid email address, such as name@example.com.");
      return;
    }
    if (normalized === emailValue.toLowerCase()) {
  ...
  async function confirmEmailChange(data: { password?: string }) {
    ...
    try {
      const response = await fetch("/api/user/update-email", {
        ...
        body: JSON.stringify({
          newEmail: shownEmail.trim().toLowerCase(),
      ...
      if (!response.ok) {
        setEmailDialogError(
          result.code === "EMAIL_EXISTS"
            ...
            : result.error || "Could not update the email address.",
        );
        return;
      }
      await user!.reload();
      ...
      setEmailDialogError(
        messageFrom(error, "Network error. Please try again."),
      );
    } finally {
      setSavingField(null);
    }
  ...
  async function updatePassword(data: {
    ...
    try {
      ...
      if (!response.ok) {
        setPasswordError(
          result.code === "form_password_incorrect"
            ? "Current password is incorrect."
            : result.code === "additional_verification_required"
              ? "Additional verification is required. Sign in again, then retry."
              : result.error || "Could not update the password.",
        );
        return;
      }
      ...
      setPasswordError(messageFrom(error, "Network error. Please try again."));
    } finally {
  ...
  async function handleFileChange(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      setImageError(messageFrom(error, "Could not upload the image."));
    } finally {
  ...
  async function removeImage() {
    ...
    try {
      ...
      await user!.reload();
      setImageSuccess("Profile photo removed.");
    } catch (error) {
      setImageError(messageFrom(error, "Could not remove the image."));
    } finally {
      setIsUploadingImage(false);
    }
  }
    ...
    try {
      ...
      if (!response.ok) {
        setDeleteError(
          result.code === "form_password_incorrect"
            ? "Current password is incorrect."
            : result.error || "Could not delete the account.",
        );
        return;
      }
      await signOut({ redirectUrl: "/" });
    } catch (error) {
      setDeleteError(messageFrom(error, "Network error. Please try again."));
    } finally {
      setIsDeleting(false);
    }
  }
    ...
    <>
      ...
      <div className="grid gap-6 xl:grid-cols-[0.72fr_1.28fr]">
        <aside className="glass-card h-fit rounded-3xl p-6 sm:p-8">
          ...
          {imageSuccess && (
            <p role="status" className="mt-4 text-xs text-emerald-700">
              {imageSuccess}
            </p>
          )}
        ...
        <section className="space-y-4">
          <ProfileField
            ...
            onSave={saveName}
            disabled={!shownName.trim() || shownName === nameValue}
          ...
          <ProfileField
            ...
            disabled={
              !shownEmail.trim() ||
              shownEmail.toLowerCase() === emailValue.toLowerCase()
            }
...
function ProfileSkeleton() {
  return (
    <div className="space-y-5">
      ...
      <div className="h-72 animate-pulse rounded-3xl bg-white/55" />
    </div>
  );
}
...
function DeleteDialog({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#061b4f]/55 p-4 backdrop-blur-sm">
      <div
      ...
      >
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="mt-2 text-sm leading-6 text-slate-500">
              All account access will be removed. This action cannot be undone.
            </p>
          </div>
          <button
            type="button"
        ...
        <input
          ...
          className={`mt-2 w-full rounded-xl border px-4 py-3 text-sm outline-none ${error ? "border-rose-300 focus:ring-2 focus:ring-rose-100" : "border-slate-200 focus:ring-2 focus:ring-blue-100"}`}
        />
        {error && (
        ...
        <div className="mt-6 flex justify-end gap-3">
          ...
          <button
            ...
            className="inline-flex min-w-32 items-center justify-center gap-2 rounded-xl bg-rose-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-rose-700 disabled:opacity-50"
          >
